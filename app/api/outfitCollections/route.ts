...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ error: 'Error fetching outfits' }, { status: 500 });
    }
}
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ message: 'Outfit added successfully', outfit: savedOutfit }, { status: 201 });
    } catch (error) {
...
// Update Outfit
export async function PUT(req: NextRequest, { params }: { params: { outfitId: string } }) {
    await connectToDB();

    const { outfitId } = params;
    const { outfitTitle, outfitSlug, outfitImage, productCollection } = await req.json();

    if (!outfitId || Array.isArray(outfitId)) {
        return NextResponse.json({ error: 'Valid Outfit ID is required' }, { status: 400 });
    }
    ...
    try {
        ...
        return NextResponse.json({ message: 'Outfit updated successfully', outfit: updatedOutfit }, { status: 200 });
    } catch (error) {
        console.error('Error updating outfit:', error);
        return NextResponse.json({ error: 'Error updating outfit' }, { status: 500 });
    }
}

// Delete Outfit
export async function DELETE(req: NextRequest, { params }: { params: { outfitId: string } }) {
    await connectToDB();
    ...
    try {
        ...
        return NextResponse.json({ error: 'Error deleting outfit' }, { status: 500 });
    }
}