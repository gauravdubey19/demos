...
//   //       selectedColor.quantity.map(q => q.size),
//   //       selectedSize,
//   //       selectedColor.color,
//   //       selectedColor.color_name,
//   //       categorySlug,
//   //       selectedColor.quantity.find(q => q.size === selectedSize)?.quantity || 0,
//   //       quantity,
//   //       selectedQuantityInfo?.quantity || 0 
//   //     );
//   //     handleClose();
//   //   }
//   // };
...
//                 {product.images_collection.map((colorOption:any, index:any) => {
...
//                       className={`w-10 h-10 rounded-full flex-center border-2 select-none ${totalQuantity === 0 ? "cursor-not-allowed opacity-40" : "hover:border-primary cursor-pointer"
...
//                 </label>
//                 <div id="size-option" className={`w-full h-fit flex gap-2 ${isValuesSelected.size && "animate-shake"}`}>
...
//                         }
//                       }}
//                       className={`w-10 h-10 rounded-full flex-center border select-none ${itemExistInCart(product._id) ? "cursor-not-allowed opacity-40" : "hover:border-primary cursor-pointer"
      ...
      >
        ...
        <div className="relative h-[240px] sm:h-[240px] md:h-[220px] lg:h-[340px] w-full flex-center select-none overflow-hidden">
          <Link
            // className="w-full h-full relative"
            href={`/products/${category}/${card.slug}`}
            className={`w-full h-full relative ${loading
                ? "bg-gray-300 animate-pulse"
                : " ease-in-out duration-300"
              }`}
          >
          ...
          {!loading && card?._id && (
            <div className="absolute z-50 left-0 right-0 bottom-1.5 lg:-bottom-40 group-hover:bottom-1.5 px-1.5 w-full h-fit ease-in-out duration-300">
              <Button
                ...
                size="sm"
                className={`w-full rounded-none hover:shadow-md disabled:opacity-80 disabled:animate-none text-lg ${loading
                    ? "bg-gray-200 animate-pulse text-gray-200"
                    : card.quantityInStock < 1
                      ? "bg-red-500"
                      : !itemExistInCart(card?._id)
        ...
        >
          <div className="h-auto w-full flex justify-between flex-col gap-1 p-1.5 md:px-2.5 mb-0.5 md:mb-1 overflow-hidden bg-white">
            <div className="">
              <div
                className={`${loading ? "h-4 md:h-7 bg-slate-200 animate-pulse" : "h-fit"
                  } w-full text-lg md:text-xl font-semibold line-clamp-1`}
              >
                {!loading && card.title}
              </div>
              <div className="h-fit w-full text-[11px] text-xs font-light text-[#818181] flex gap-1">
                ...
                {reviews > 0 && (
                  <>
                    |{" "}
                    <span
                      className={`line-clamp-1 ${loading &&
                        "mt-1 w-20 h-2 md:h-3 bg-gray-200 animate-pulse"
                        }`}
                    >
                      {!loading && reviews + " reviews"}
              ...
              <div
                className={`w-full text-[11px] md:text-xs text-[#818181] line-clamp-1 ${loading ? "mt-2 h-7 bg-gray-200 animate-pulse" : "h-fit"
                  }`}
              >
                {!loading && card.description}
              </div>
            ...
            <div className="flex flex-col lg:flex-row lg:gap-2 lg:items-end">
              <span
                className={`${loading && "w-20 h-5 md:h-7 bg-gray-200 animate-pulse"
                  } text-lg md:text-xl font-bold`}
              >
                {!loading && "₹" + card.price}
              </span>
              {card.oldPrice && (
                <div className="flex flex-wrap gap-2">
                  <span
                    className={`text-xs md:text-md line-through text-gray-400 ${loading &&
                      "mt-1 lg:mt-0 w-14 h-3 md:h-5 bg-gray-200 animate-pulse"
                      }`}
                  >
                    {!loading && "₹" + card.oldPrice}
      ...
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="w-full h-fit flex flex-col gap-8">
          ...
          <div className="grid gap-4">
              ...
              >
                {product.availableSizes.map((s) => (
                  <div
                    ...
                    className={`w-10 h-10 rounded-full flex-center border select-none ${itemExistInCart(product._id)
                        ? "cursor-not-allowed opacity-40"
                        : size === s
                          ? "border-primary shadow-lg scale-105 cursor-not-allowed"
                          : "hover:border-primary cursor-pointer"
            ...
            <div className="flex flex-col gap-y-2">
              <span
                className={`text-sm py-2 px-2 ${product.quantityInStock > 5
                    ? "text-green bg-emerald-50"
                    : "text-red-500 bg-red-50"
                  }`}
              >
