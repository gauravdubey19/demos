...
const ProductDetail: React.FC<{ slug: string; categorySlug: string }> = ({
  ...
  const [selectedColorData, setSelectedColorData] = useState<{
    ...
    image_link: string; // Add this line
    color: string;
    color_name: string;
    images: string[];
  ...
  useEffect(() => {
    const fetchProductBySlug = async () => {
      try {
        ...
        // Set initial color data if available
        if (data.product.images_collection.length > 0) {
          const initialColor = data.product.images_collection[0]; // Get the first color
          setSelectedColorData({
            _id: initialColor._id,
            image_link: initialColor.image_link,
            color: initialColor.color,
            color_name: initialColor.color_name,
            images: initialColor.images,
            quantity: initialColor.availableSizes // Make sure this is correctly set
          });
          setCurrentImage(initialColor.images[0]); // Set the current image to the first image of the selected color
        }
      } catch (error) {
      ...
      <section className="w-full h-full max-w-6xl px-4 py-10 mx-auto">
        <div className="w-full h-full mt-8 lg:mt-[80px] xl:mt-10">
          ...
          <div className="w-full h-full grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-6 lg:gap-12 items-start lg:mt-2">
            <ImageGalleryMobile images={selectedColorData?.images || []} />
            <ImageGallery
              images={selectedColorData?.images || []}
              initialImageLink={currentImage}
            />
            <Details
              product={product}
  ...
  return (
    <div className="hidden md:flex relative z-50 select-none lg:sticky lg:top-20 w-full h-full md:h-[50vh] lg:h-[85vh] flex-col gap-3 md:flex-row-reverse justify-between overflow-hidden lg:overflow-visible">
      ...
      <div className="relative w-full h-full md:w-[20%] flex flex-col">
        <div
          ref={thumbnailRef}
          className={`absolute h-full flex ${isMobileView ? "flex-row" : "flex-col"
        ...
        >
          {images.map((image:any, index:any) => (
            <Image
              ...
              width={200}
              height={200}
              className={`cursor-pointer lg:w-full lg:h-fit md:w-full md:h-fit w-40 h-40 ${currentImage === image &&
        ...
        {showArrowLeft && (
          <div
            ...
            className={`absolute top-0 z-10 flex-center group cursor-pointer ${isMobileView
        ...
        {showArrowRight && (
          <div
            ...
            className={`absolute bottom-0 z-10 flex-center group cursor-pointer ${isMobileView
...
interface DetailsProps {
  product: Product;
  categorySlug: string;
  selectedColorData: ImageCollection | null;
  setSelectedColorData: (data: ImageCollection) => void;
  setCurrentImage: (image: string) => void;
  avgRating: number; // Add this line
  reviewsLength: number; // Add this line
}
...
const Details: React.FC<DetailsProps> = ({
  ...
  const handleColorSelect = (colorData: ImageCollection) => {
    setSelectedColorData({
      _id: colorData._id, // Add this line
      image_link: colorData.image_link, // Add this line
      color: colorData.color,
      color_name: colorData.color_name, // Changed from colorName to color_name
      images: colorData.images,
      quantity: colorData.quantity
    });
    setCurrentImage(colorData.images[0]);
    setSize("");
  ...
  return (
    <div className="relative w-full h-full grid gap-3">
      ...
      <div className="flex items-end gap-2">
        <ReactCountUp
          prefix="₹"
          amt={product.sale_price || product.price}
          decimals={true}
          className="text-4xl font-bold"
        />
        {product.sale_price && (
          <ReactCountUp
            amt={((product.price - product.sale_price) / product.price) * 100}
      ...
      {product.reviews_number > 0 ? (
        <div className="flex items-center gap-1 md:gap-4">
          ...
          |{" "}
          <div>
            <span className="text-primary">{product.reviews_number}</span> reviews
          </div>
        </div>
      ) : (
      ...
      <div className="grid gap-4">
        ...
        <div className="grid gap-2">
          ...
          <div
            id="color-option"
            className={`w-full h-fit flex gap-2 ${isValuesSelected.color && "animate-shake"
              }`}
          >
            {product.images_collection.map((colorData: ImageCollection) => (
              <div
                key={colorData.color}
                title={colorData.color_name}
                onClick={() => handleColorSelect(colorData)}
                style={{ backgroundColor: colorData.color }}
                className={`w-10 h-10 rounded-full flex-center border-2 select-none ${itemExistInCart(product._id)
        ...
        <div className="grid gap-2">
          <label
            htmlFor="size"
            className={`text-base font-medium ${isValuesSelected.size && "text-[red]"} ease-in-out duration-200`}
          >
          ...
          <div
            id="size-option"
            className={`w-full h-fit flex gap-2 ${isValuesSelected.size && "animate-shake"}`}
          >
            {/* Check if selectedColorData and its quantity are available before mapping */}
            {selectedColorData && selectedColorData.quantity ? (
              selectedColorData.quantity.map((sizeData: ProductQuantity) => (
                <div
                  key={sizeData.size}
                  onClick={() => {
                    if (!itemExistInCart(product._id) && sizeData.quantity > 0) {
                      setSize(sizeData.size);
                      setIsValuesSelected((prev) => ({ ...prev, size: false }));
                    }
                  }}
                  className={`w-10 h-10 rounded-full flex-center border select-none ${sizeData.quantity === 0
                    ...
                    : itemExistInCart(product._id)
                      ...
                      : size === sizeData.size
                        ? "border-primary shadow-lg scale-105 cursor-not-allowed"
                        : "hover:border-primary cursor-pointer"
                    } ${isValuesSelected.size && "border-[red]"} ease-in-out duration-300`}
                  title={
                    sizeData.quantity === 0
                      ? "Out of stock"
              ...
              <div className="text-muted-foreground">No sizes available</div> // Fallback message
            )}
          </div>
        </div>
  ...
  const allSizes = Array.from(
    new Set(
      product.images_collection
        .flatMap(item => item.quantity.map(q => q.size))
    )
  ).sort();

  // Get all color names
  const allColors = product.images_collection.map(item => item.color_name);
  ...
  return (
    <>
      ...
      <Accordion type="single" collapsible className="w-full">
        ...
        <AccordionItem value="item-1">
          ...
          <AccordionContent className="bg-gray-100/50 text-sm p-4 rounded-md">
            <ul className="list-disc pl-5 space-y-2">
              ...
              {product.fabric_type && (
                <li>
                  <strong>Fabric Type:</strong> {product.fabric_type}
                </li>
              )}
              {product.pattern && (
                <li>
        ...
        {/* New Product Highlights Accordion */}
        ...
        {/* Available Sizes Accordion */}
        <AccordionItem value="item-3">
          <AccordionTrigger>Available Sizes</AccordionTrigger>
          <AccordionContent className="bg-gray-100/50 text-sm p-4 rounded-md">
            <ul className="list-disc pl-5 space-y-2">
              <li>
                <strong>Available Sizes:</strong> {allSizes.join(", ")}
              </li>
              <li>
                <strong>Color Options:</strong> {allColors.join(", ")}
              </li>
              {product.type && product.type.length > 0 && (
                <li>
                  <strong>Type:</strong> {product.type.join(", ")}
                </li>
              )}
            </ul>
          </AccordionContent>
                      ...
                      {handleEditId === review._id.toString() ? (
                        <div className=" flex items-center">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <IoMdStar
                              key={star}
                              className={`w-6 h-6 cursor-pointer ${star <= editRating
                                  ? "fill-primary"
                                  : "fill-gray-500"
                                }`}
