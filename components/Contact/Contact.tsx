...
//             <Button className={``} type="submit">{Loading?"Sending..":"Send"}</Button>
...
import { ChevronRight } from "lucide-react";
import { Button } from "../ui/button";
import { toast } from "@/hooks/use-toast";; // Ensure Toast component is properly imported
...
const Contact = () => {
  ...
  const [errorMessage, setErrorMessage] = useState("");
  const [loading, setLoading] = useState(false); // Use lowercase for 'loading'
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      toast({
        ...
        variant: "default", // Make sure you use a valid variant here
      });
    } catch (error: any) {
      // Show error toast
      toast({
