...
const Analysis = ({ data }: AnalysisProps) => {
    ...
    const pieChartData = {
        ...
        datasets: [
            {
                label: "Issues",
                // Map the open and closed counts to the chart data
                data: [data.openCount, data.closedCount],
                backgroundColor: ["#ffb433", "#2ED395"],
                hoverOffset: 4,
            },
        ],
    };
    ...
    const pieChartOptions: ChartOptions<"pie"> = {
        ...
        plugins: {
            ...
            tooltip: {
                callbacks: {
                    label: (context: any) => `${context.label}: ${context.raw} issues`, // Show issue count in tooltip
                },
                backgroundColor: "#FFAC28", // Tooltip background color
                bodyFont: { size: 12 },
                bodyColor: "#fff", // White text for tooltip
    ...
    return (
        <div className="bg-[#f8f8f8] p-6 rounded-lg shadow-md mb-6 border">
            <h2 className="text-xl font-semibold mb-4">Issues Analysis</h2>
            <div className="flex items-center">
                <div className="w-1/3 h-40 pl-20">
                    <Pie data={pieChartData} options={pieChartOptions} />
                </div>
                <div className="w-2/3 pl-4">
                    <p className="flex items-center mb-2">
                        <span className="inline-block w-3 h-3 bg-orange-300 rounded-full mr-2"></span>
                        No. of open issues: <span className="ml-auto">{data.openCount}</span>
                    </p>
                    <p className="flex items-center mb-2">
                        <span className="inline-block w-3 h-3 bg-[#2ED395] rounded-full mr-2"></span>
                        No. of closed issues: <span className="ml-auto">{data.closedCount}</span>
                    </p>
                    <hr className="my-2" />
                    <p className="flex items-center">
                        Total Issues: <span className="ml-auto">{data.total}</span>
                    </p>
                </div>
            </div>
        </div>
